...
/* ====== Part B: Sermons system ====== */

/*
  Behavior:
  - Loads sermons from sermons.json in the repo root
  - Renders list with small audio players and a main player
  - Only one audio plays at a time
  - Search input filters sermons
  - Simple password-protected upload form that generates a JSON object to copy into sermons.json
*/
const ADMIN_PASSWORD = "edy123"; // <<-- change if you want another password
const SERMONS_JSON = "sermons/sermons.json"; // now inside sermons folder
const SERMONS_FOLDER = "sermons/"; // folder where mp3 files are stored

// UI elements
const tabHymns = document.getElementById('tab-hymns');
const tabSermons = document.getElementById('tab-sermons');
const tabUpload = document.getElementById('tab-upload');

const hymnsSection = document.getElementById('hymns-section');
...
const dayInput = document.getElementById('u-day');

let sermonsData = [];

// tab switching
function showTab(name) {
  tabHymns.classList.remove('active');
  tabSermons.classList.remove('active');
  tabUpload.classList.remove('active');
  hymnsSection.classList.add('hidden');
  sermonsSection.classList.add('hidden');
  uploadSection.classList.add('hidden');

  if (name === 'hymns') {
    tabHymns.classList.add('active');
    hymnsSection.classList.remove('hidden');
    ...
    uploadFormWrap.classList.add('hidden');
    adminPasswordInput.value = '';
  }
}

tabHymns.addEventListener('click', e => { e.preventDefault(); showTab('hymns'); });
tabSermons.addEventListener('click', e => { e.preventDefault(); showTab('sermons'); });
tabUpload.addEventListener('click', e => { e.preventDefault(); showTab('upload'); });

// auto-fill day name when picking a date
...
// fetch sermons.json, sort by date desc, render
async function loadSermonsJSON() {
  try {
    const res = await fetch(SERMONS_JSON, { cache: "no-store" });
    if (!res.ok) throw new Error("sermons.json not found or fetch failed");
    const data = await res.json();

    sermonsData = data.sort((a, b) => new Date(b.date) - new Date(a.date));
    renderSermons(sermonsData);
  } catch (err) {
    sermonsListEl.innerHTML = `<li class="sermon-item note">خطأ في تحميل sermons.json — تأكد من وجود الملف في المستودع. (${err.message})</li>`;
    console.error(err);
  }
}

function renderSermons(list) {
  sermonsListEl.innerHTML = "";
  if (!list.length) {
    sermonsListEl.innerHTML = `<li class="sermon-item note">لا توجد عظات مضافة بعد.</li>`;
    mainSource.src = "";
    mainAudio.load();
  ...
  list.forEach((s, idx) => {
    ...
    const titleText = `${s.date} — ${s.day} — ${s.bible}`;
    li.innerHTML = `
      <div class="meta">${escapeHtml(titleText)}</div>
      <audio controls preload="none" data-idx="${idx}">
        <source src="${SERMONS_FOLDER + encodeURIComponent(s.file)}" type="${mimeTypeForFile(s.file)}">
        متصفحك لا يدعم تشغيل الصوت
      </audio>
    `;
    li.querySelector('.meta').addEventListener('click', () => loadIntoMainPlayer(s));
    sermonsListEl.appendChild(li);
  });
...
function loadIntoMainPlayer(s) {
  const fileUrl = SERMONS_FOLDER + s.file;
  mainSource.src = fileUrl;
  mainSource.type = mimeTypeForFile(s.file);
  mainAudio.load();
  mainAudio.play().catch(() => {});
  currentTitle.textContent = `${s.date} — ${s.day} — قراءة: ${s.bible}`;
  formatNote.style.display = (s.file || '').toLowerCase().endsWith('.opus') ? 'block' : 'none';
}

function setupSinglePlay() {
  const audios = Array.from(sermonsListEl.querySelectorAll('audio'));
  audios.forEach(a => {
    a.addEventListener('play', () => {
      audios.forEach(other => {
        if (other !== a) other.pause();
      });
      if (!mainAudio.paused && mainAudio.src !== a.querySelector('source').src) mainAudio.pause();
    });
  });
  mainAudio.addEventListener('play', () => {
    Array.from(sermonsListEl.querySelectorAll('audio')).forEach(a => { if (!a.paused) a.pause(); });
  });
}

// search filter
sermonSearch.addEventListener('input', () => {
  const q = sermonSearch.value.trim().toLowerCase();
  if (!q) return renderSermons(sermonsData);
  renderSermons(sermonsData.filter(s => {
    return `${s.date} ${s.day} ${s.bible} ${s.file}`.toLowerCase().includes(q);
  }));
});

// Suggest next sermon(n).mp3 filename
function suggestNextFileName() {
  try {
    const fileInput = document.getElementById('u-file');
    if (!fileInput) return;

    if (!sermonsData.length) {
      fileInput.value = "sermon1.mp3";
      return;
    }

    let maxNum = 0;
    sermonsData.forEach(s => {
      const match = s.file.match(/^sermon(\d+)\.mp3$/i);
      if (match) {
    ...
    fileInput.value = `sermon${maxNum + 1}.mp3`;
  } catch (e) {
    console.error("suggestNextFileName error:", e);
  }
}
...
adminEnterBtn.addEventListener('click', () => {
  if (adminPasswordInput.value === ADMIN_PASSWORD) {
    uploadFormWrap.classList.remove('hidden');
    suggestNextFileName();  // <-- call here, after showing form
  } else {
...
generateJsonBtn.addEventListener('click', () => {
  ...
  const bible = document.getElementById('u-bible').value.trim();
  const file = document.getElementById('u-file').value.trim();
  if (!d || !day || !bible || !file) {
    alert('يرجى تعبئة كل الحقول!');
    return;
  }
  const obj = { date: d, day: day, bible: bible, file: file };
  jsonOutput.value = JSON.stringify(obj, null, 2);
  jsonOutput.scrollIntoView({ behavior: 'smooth' });
});

copyJsonBtn.addEventListener('click', async () => {
  if (!jsonOutput.value) return alert('No JSON to copy');
  if (navigator.clipboard && window.isSecureContext) {
    try {
      await navigator.clipboard.writeText(jsonOutput.value);
      return alert('JSON copied to clipboard!');
    } catch {}
  }
  jsonOutput.select();
  try {
